...
<div class="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-800 tracking-tight">Edit Akun User</h1>
            <p class="text-slate-500 mt-1 font-medium italic">Perbarui informasi profil dan hak akses pengguna.</p>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
        <form action="{{ route('user.update', $user->id) }}" method="POST" class="space-y-8">
                    ...
                    <div>
                        ...
                        <select name="level" id="level" required
                            ...
                            @foreach($levels as $lv)
                                <option value="{{ $lv }}" {{ old('level', $user->level) == $lv ? 'selected' : '' }}>{{ ucfirst($lv) }}</option>
                            @endforeach
                        </select>
            ...
            <div class="space-y-6 pt-4 border-t border-slate-50">
                <p class="text-[10px] font-bold text-rose-600 uppercase tracking-widest border-b border-rose-50 pb-2">Keamanan (Kosongkan jika tidak ingin mengubah)</p>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
