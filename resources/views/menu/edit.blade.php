...
<div class="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-800 tracking-tight">Edit Menu</h1>
            <p class="text-slate-500 mt-1 font-medium italic">Perbarui item navigasi sidebar.</p>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
        <form action="{{ route('menu.update', $menu->id) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $menu->name) }}" required placeholder="Contoh: Dashboard"
                    ...
                    <div>
                        ...
                        <input type="text" name="url" id="url" value="{{ old('url', $menu->url) }}" required placeholder="Contoh: dashboard atau /admin/settings"
            ...
            <div class="space-y-2">
                ...
                <textarea name="icon" id="icon" rows="4" placeholder="Tempel kode SVG <svg>...</svg> di sini"
                    class="w-full px-4 py-3 rounded-2xl border border-slate-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all font-mono text-xs text-slate-500 bg-slate-50/50">{{ old('icon', $menu->icon) }}</textarea>
